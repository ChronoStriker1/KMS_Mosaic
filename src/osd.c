...
static void render_text_to_rgba(font_ctx *f, const char *text, unsigned char **out, int *w, int *h) {
    ...
    *w = max_w ? max_w : 1; *h = lines * line_h;
    ...
    int x=0,y=0; for (const unsigned char *p=(const unsigned char*)text; *p; ++p){
        ...
        for (int yy=0; yy<g->bitmap.rows; yy++){
            int py = gy + yy; if (py<0 || py>=*h) continue;
            for (int xx=0; xx<g->bitmap.width; xx++){
...
static GLuint compile_shader(GLenum type, const char *src){ GLuint s=glCreateShader(type); glShaderSource(s,1,&src,NULL); glCompileShader(s); GLint ok; glGetShaderiv(s,GL_COMPILE_STATUS,&ok); if(!ok){ char log[256]; glGetShaderInfoLog(s,sizeof log,NULL,log); fprintf(stderr,"osd shader: %s\n",log);} return s; }
static void ensure_prog(void){ if(osd_prog) return; const char* vs="attribute vec2 a_pos; attribute vec2 a_uv; varying vec2 v_uv; void main(){ v_uv=a_uv; gl_Position=vec4(a_pos,0,1);}"; const char* fs="precision mediump float; varying vec2 v_uv; uniform sampler2D u_tex; void main(){ gl_FragColor=texture2D(u_tex,v_uv);}"; GLuint v=compile_shader(GL_VERTEX_SHADER,vs), f=compile_shader(GL_FRAGMENT_SHADER,fs); osd_prog=glCreateProgram(); glAttachShader(osd_prog,v); glAttachShader(osd_prog,f); glBindAttribLocation(osd_prog,0,"a_pos"); glBindAttribLocation(osd_prog,1,"a_uv"); glLinkProgram(osd_prog); osd_u_tex=glGetUniformLocation(osd_prog,"u_tex"); glGenBuffers(1,&osd_vbo);} 
